...
- Provider status, fresh/open/send/collect responses can include best-effort provider conversation metadata such as status, marker type, marker hash, and capture time. This is an observability surface for trusted orchestration; the bridge must not decide whether an external agent should reuse a provider conversation.
