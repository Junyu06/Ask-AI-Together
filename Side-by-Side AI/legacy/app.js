...
const I18N = {
  zh: {
    ...
    usage_issues: "问题反馈",
    about_subtitle: "支持项目",
    about_support_text: "如果这个扩展对你有帮助，欢迎请我喝咖啡。",
  ...
  en: {
    ...
    usage_issues: "Issues",
    about_subtitle: "Support the project",
    about_support_text: "If this extension helps you, consider buying me a coffee.",
    buy_me_a_coffee: "Buy Me a Coffee",
    save: "Save",
    add_site: "Add Site",
    site_name: "Site Name",
...
async function loadLaunchMode() {
  const data = await chrome.storage.local.get([STORAGE_KEYS.launchMode]);
  const saved = data[STORAGE_KEYS.launchMode];
  launchMode = ["legacy", "windows"].includes(saved) ? saved : "legacy";
}
...
function syncLaunchModeRadio() {
  const radio = document.querySelector(`input[name="launch-mode"][value="${launchMode}"]`);
  if (radio) radio.checked = true;
}
