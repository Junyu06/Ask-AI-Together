...
const legacyContentSource = fs.readFileSync(legacyContentPath, "utf8");
assert.doesNotMatch(
  legacyContentSource,
  /function\s+(extractLatestResponseText|collectReplyNodes)\s*\(/,
  "legacy content must not redeclare response extraction helpers"
);
const source = `${fs.readFileSync(contentResponsePath, "utf8")}
globalThis.__legacyResponseTestApi = {
...
function makeContext() {
  const context = vm.createContext({
    ...
    chrome: {
      runtime: {
        getURL() {
          return "chrome-extension://ask-ai-together/";
        }
      }
    },
    location: {
...
vm.runInContext(source, context, { filename: legacyContentPath });
const api = context.__legacyResponseTestApi;
...
setPage(
  ...
  el("main", {}, "", {}, [
    ...
    el("article", {}, "", { top: 10, bottom: 30 }, [
      el("div", { "data-message-author-role": "assistant" }, "OK LEGACY", { top: 10, bottom: 30 })
    ])
  ])
);
assert.equal(api.extractLatestResponseText(), "OK LEGACY");
...
setPage(
  ...
  el("main", {}, "", {}, [
    ...
    el("div", {}, "", { top: 40, bottom: 60 }, [
      el("div", { class: "prose" }, "OK LEGACY", { top: 40, bottom: 60 })
    ])
  ])
);
assert.equal(api.extractLatestResponseText(), "OK LEGACY");
...
setPage(
  ...
  el("main", {}, "", {}, [
    el("textarea", { "aria-label": "Ask" }, "", { top: 90, bottom: 110 }),
    el("div", { "data-testid": "assistant-message" }, "", { top: 20, bottom: 40 }, [
      el("div", { class: "prose" }, "Analyzing...", { top: 20, bottom: 40 })
    ])
  ])
);
assert.equal(api.extractLatestResponseText(), "");
...
console.log("legacy response extraction validation passed");
