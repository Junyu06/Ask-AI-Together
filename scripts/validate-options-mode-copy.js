...
for (const source of [legacyAppSource, optionsI18nSource]) {
  ...
  assert.match(source, /mode_hint:\s*"先使用默认模式。Perplexity 默认模式用不了，需要兼容模式。其他 AI 网站如果打不开、空白或不能正常使用，也切换到兼容模式。"/);
  ...
  assert.match(
    source,
    /mode_hint:\s*"Use Default mode first\. Perplexity does not work in Default mode, so use Compatibility mode for it\. If another AI site does not open, stays blank, or does not work correctly, switch to Compatibility mode too\."/
  );
...
for (const source of [legacyIndexSource, optionsHtmlSource]) {
  ...
  assert.ok(source.includes("先使用默认模式。Perplexity 默认模式用不了，需要兼容模式。其他 AI 网站如果打不开、空白或不能正常使用，也切换到兼容模式。"));
