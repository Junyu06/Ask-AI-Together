...
- 在主输入框支持粘贴/拖入文件。
- 每次附件操作会替换当前待发附件（不累积历史批次）。
- 粘贴后立即广播 `ATTACH_FILES` 给目标站点，做“预加载上传”。
- `CHAT_MESSAGE` 当前只发文本；发送阶段不再二次附文件。
- 内容页附文件顺序：`file input` -> `drop` -> `paste`。
...
- 附件消息也会入历史（无文本时使用 `[附件]`/`[Attachment]`）。
